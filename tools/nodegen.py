    ...
    with open(node_source_path, "w") as file:
        ...
        def write_struct(struct):
            ...
            if fields:
                ...
                parent_parameters = parent_fields
                # add underscore to prevent ambiguity between parameter and field
                unique_parameters = list(
                    map(
                        lambda p: [p[0], "_" + p[1] if p in unique_fields else p[1]],
                        unique_fields,
                    )
                )
                ...
                # signature
                joined_params = list(
                    map(" ".join, parent_parameters + unique_parameters)
                )
                file.write(f"\n{name}::{name}({', '.join(joined_params)})\n")

                # initializer list
                parent_arguments = list(map(init_field, parent_parameters))
                unique_arguments = list(map(init_field, unique_parameters))
                initializer_list = []
                if parent_fields:
                    initializer_list.extend(
                        [f"{struct.parent}({arg})" for arg in parent_arguments]
                    )
